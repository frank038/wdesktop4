...
class customWidget(Gtk.Widget):
    def __init__(self, _parent, _path, _width, _height):
        ...
        self._style_context.add_class("wnotes")
    ...
    def _populate(self):
        ...
        self.main_box.append(_scrolled)
        _scrolled.set_vexpand(True)
        _scrolled.set_propagate_natural_height(True)
        _scrolled.set_propagate_natural_width(True)
        #
        self.text_view = Gtk.TextView()
        self.text_view.set_wrap_mode(Gtk.WrapMode.WORD_CHAR)
        _scrolled.set_child(self.text_view)
        self.text_buffer = self.text_view.get_buffer()
        self.note_path = os.path.join(self._parent.CURRENT_DIR,"widgets",self._module,"note.txt")
        try:
            if os.path.exists(self.note_path):
                with open(self.note_path, "r") as _f:
                    self.text_buffer.set_text(_f.read())
        except:
            pass
        #
        save_btn = Gtk.Button(label="Save")
        self.main_box.append(save_btn)
        save_btn.connect("clicked", self.on_save_note)
        
    def on_save_note(self, btn):
        textview_text = self.get_textview_text()
        if textview_text != None or textview_text != "":
            try:
                with open(self.note_path, "w") as _f:
                    _f.write(textview_text)
            except:
                pass
